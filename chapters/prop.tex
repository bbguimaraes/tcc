...
A comparação do desempenho de diferentes algoritmos deve ser baseada em critérios sólidos para que tenha algum valor prático. Assim, nesse trabalho, o método para comparação será o seguinte (os conceitos usados nessa seção são explicados no capítulo \ref{chap:eval}):
...
\begin{enumerate}
    ...
    \item Calcular, para o resultado de cada algoritmo:
        \begin{enumerate}
            ...
            \item O índice de Youden com significância maior para os falsos negativos (seção.
        \end{enumerate}
\end{enumerate}

O erro médio relativo é uma medida padrão simples, que mostra o quanto o resultado de um teste difere dos valores corretos. Ele é utilizado como uma métrica simples para o desempenho de um algoritmo. No entanto, a simplicidade do cálculo do erro médio relativo não é suficiente para a análise de áreas mais complexas como a detecção de fraude.
...
Conforme o padrão nesse tipo de experimento, os dados serão apresentados em formato de tabela e gráfico. Como será apresentado em uma seção posterior, os critérios escolhidos já são parte integrante dos resultados exibidos na execução do WEKA. Essa escolha reflete tanto a aderência dessa plataforma aos padrões da área como a facilidade que uma ferramenta desse tipo oferece na determinação do método de avaliação.
